...
public class BaseAction extends ActionSupport
	...
	public <T> PageBean pageQuery(Class<T> entityClass) throws ActionException {
		...
		PageBean pageBean = baseModelService.pageQuery(bulidConditionSql(),
				bulidOrderBySql(), Integer.valueOf(pageSize),
				Integer.valueOf(page), entityClass);
	 ...
	 * @throws DaoException
	 */
	public <T> List<T> query(Class<T> entityClass) throws ActionException,
			DaoException {
		if (entityClass == null)
			throw new ActionException("entityClass must be defined in Action");
	 ...
	 * @throws DaoException
	 */
	public <T> List<T> queryByCache(Class<T> entityClass)
			throws ActionException, DaoException {
		if (entityClass == null)
			throw new ActionException("entityClass must be defined in Action");
	...
	public <T> void forwardAsContent(String page, String container,
			Map<String, T> dataModel) {
		if (dataModel != null) {
			for (String key : dataModel.keySet()) {
				request.setAttribute(key, dataModel.get(key));
			}
	 ...
	 *            模板參數
	 */
	public <T> void render(String template, String container,
			Map<String, T> dataModel) {
		if (dataModel != null)
	...
	public <T> void render(String template, Map<String, T> dataModel) {
		try {
			...
			for (String name : sessionName) {
				dataModel
						.put(name, (T) request.getSession().getAttribute(name));
			}
			// 模板中传入request值
			Map paramMap = request.getParameterMap();
			for (Iterator itor = paramMap.keySet().iterator(); itor.hasNext();) {
