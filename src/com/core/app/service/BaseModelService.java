...
public class BaseModelService {
	...
	public PageBean pageQuery(String condition, String orderBy,
			Integer pageSize, Integer page, Class entityClass) {
	...
	public String buildSQLWhere(Map map) {
		...
		for (Iterator<String> itor = map.keySet().iterator(); itor.hasNext();) {
			...
			if (keystr.startsWith("_or") && keystr.length() > 9
					...
					&& keystr.substring(4, 5).equals("_")
					&& keystr.substring(6, 7).equals("_")) {
			...
			if (key.startsWith("_")) {
				if (key.length() > 6
						&& ("_eq_,_le_,_ge_,_lt_,_gt_,_ne_,_in_,_ni_")
								.indexOf(key.substring(2, 6)) >= 0) {
					value = (String) map.get(keystr);
					key = key.substring(6);
				} else if (key.length() > 9
						&& ("_exist_,_llike_,_rlike_,_nlike_,_likes_")
								.indexOf(key.substring(2, 9)) >= 0) {
					value = (String) map.get(keystr);
					key = key.substring(9);
				} else if (key.length() > 10
						&& ("_nexist_").indexOf(key.substring(2, 10)) >= 0) {
					value = (String) map.get(keystr);
					key = key.substring(10);
				} else if (key.length() > 8
						&& ("_like_").indexOf(key.substring(2, 8)) >= 0) {
					value = (String) map.get(keystr);
					key = key.substring(8);
				} else if (key.length() > 10
						&& ("_isnull_").indexOf(key.substring(2, 10)) >= 0) {
					value = (String) map.get(keystr);
					key = key.substring(10);
				}
			} else {
				...
				if (orflag == null) {
					...
					if (symbol.startsWith("_in_") || symbol.startsWith("_ni_")) {
						sb.append(")");
							...
							|| symbol.startsWith("_exist_")) {
						sb.append(")");
					} else if (symbol.startsWith("_llike_")) {
						sb.append("'");
					} else if (symbol.startsWith("_rlike_")
							|| symbol.startsWith("_nlike_")
